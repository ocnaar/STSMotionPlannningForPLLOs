...
% Stand (STS) movement of a Powered Lower Limb Orthosis. The joints of the
% orthosis are controlled via feedback linearization to perform a rest- 
% to-rest maneuver between the sitting position defined by zi and the 
% standing position defined by zf. 
...
%
% tf: duration of the STS ascension phase. 
%
% x: System States
...
function xdot = FLThreeLink(t,tf,x,zi,zf,pnom,punc)
...
% Vector for feedback linearization of the system.
b = M*(v+thd(7:end))-f;
...
uctrl = lsqlin(W,zeros(size(W,1),1),[],[],Atau,b,lbu,ubu,[],options);
